...
model.load_weights('modelu2net_13_40epochs_filled_multiclass.h5')
...
pred_data = []
true_mask = []
for files in sorted(os.listdir('multiclass_data_filled/test/images/')):
    # print(files)
    if(files[0]!='.'):
        img = cv2.imread('multiclass_data_filled/test/images/'+files,0)
        img = cv2.resize(img,(512,512))
        img = img.reshape((1,512,512,1))/255
        img = 2*img - 1
        preds = model.predict(img)
        # print(preds.shape)
        # img_data.append(img)

        pred_data.append(cv2.cvtColor(onehot_to_rgb(preds[0].reshape((512,512,5)),id2code), cv2.COLOR_BGR2RGB))
...
def create_tile(img_list):
    ...
    for y in range(0,px,step): #no need to sub 512 b/c px are mult of 512
        for x in range(0,px,step):
            ...
            # count += 1
            # break
    cv2.imwrite('u2net_13_40epochs_filled_multiclass_bce.png',tile)
    return tile
