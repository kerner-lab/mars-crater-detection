...
model = UNetFormer(pretrained=False,num_classes=1)
model.to(0)
...
checkpoint = torch.load('unetformer_13_40epochs_edges.pth')
...
for files in sorted(os.listdir('test_1/images/')):
    if(files[0]!='.'):
      img = cv2.imread('test_1/images/'+files)
...
def create_tile(img_list):
    ...
    for y in range(0,px,step): #no need to sub 512 b/c px are mult of 512
        for x in range(0,px,step):
            ...
            # count += 1
            # break
    cv2.imwrite('unetformer_21_40epochs_edges.png',tile)
    return tile
