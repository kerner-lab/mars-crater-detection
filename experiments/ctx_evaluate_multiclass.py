...
img = PIL.Image.open('MurrayLab_GlobalCTXMosaic_V01_E-008_N-44/MurrayLab_CTX_V01_E-008_N-44_Mosaic.tif')
img = np.array(img)

px = 47104
step=512 
newpx = 512
out_img = np.zeros((px,px,3))
...
model.load_weights('modelu2net_mc_40_epochs_gs.h5')
...
for y in range(0,px,step): #no need to sub 512 b/c px are mult of 512
    for x in range(0,px,step):
        ...
        # print(temp_img.shape)
        preds = model.predict(temp_img.reshape((1,512,512,1))/255,verbose=0)
        # preds = preds[0].reshape((512,512))
        preds = onehot_to_rgb(preds[0].reshape((512,512,5)),id2code)
        preds = cv2.resize(preds,(newpx,newpx))
        out_img[x:x+newpx,y:y+newpx,:] = preds
        count += 1
        if(count%500==0):
            print('processed',count)
        # break
    # break
cv2.imwrite('ctx_out_512_multilcass.png',out_img)