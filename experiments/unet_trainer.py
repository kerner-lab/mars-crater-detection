...
checkpoint = ModelCheckpoint(
    'trained_models/unet_13_500epochs.h5',
    monitor='val_loss',
    verbose=0,
    save_best_only=True,
...
train = Mygenerator('out_new_split/train/images','out_new_split/train/mask',10)
val = Mygenerator('out_new_split/val/images','out_new_split/val/mask',10)
...
pred_data = []
true_mask = []
for files in sorted(os.listdir('out_new_split/test/images/')):
    # img = cv2.imread('out_new_split/test/images/'+files,0)
    img = plt.imread('out_new_split/test/images/'+files)
    img = 2*img - 1
    # img = cv2.resize(img,(512,512))
...
def create_tile(img_list):
    ...
    for y in range(0,px,step): #no need to sub 512 b/c px are mult of 512
        for x in range(0,px,step):
            ...
            # count += 1
            # break
    cv2.imwrite('generated_masks/unet_13_500epochs.png',tile)
    return tile
